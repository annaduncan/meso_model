'''
    ...
    12 OCt 2020 
    Updated script for use with python 3
...
def check_for_clustering(dist, x_coords, y_coords, angles, n_particles, cutoff):
    ...
    proteins_array = np.array(list(zip(x_coords, y_coords, np.zeros(n_particles,dtype=np.float32))))
...
def parse_gro_file(start_struct, cutoff, cluster_text, sticky_patches, sticky_patches_range, verbose, PBC):
    ...
    proteins_array = np.array(list(zip(x_coords, y_coords, np.zeros(natoms,dtype=np.float32))))
    if PBC:
...
#        print 'setup took: {} seconds'.format(currentTime1 - startTime)
    ...
    for i in range(n_clusters):
        if PBC:
            cluster_coords = np.array(list(zip(old_x_coords[cluster_list == i], old_y_coords[cluster_list == i])))  # NOTE: use the copies of the ORIGINAL coords            
...
#        print 'finding cluster CoG took: {} seconds'.format(currentTime2 - currentTime1)
    ...
    if PBC:
        closest_coords = np.array(list(zip(old_x_coords, old_y_coords)))
...
#        print 'translating particles took:  {} seconds'.format(currentTime5 - currentTime4)
    ...
    clashing_clusters = list(range(n_clusters))
...
#            print 'iteration:  {} '.format(iteration)
        proteins_array = np.array(list(zip(new_x_coords, new_y_coords, np.zeros(n_particles,dtype=np.float32))))
        if iteration > 0:
            clashing_proteins_array = np.array(list(zip(new_x_coords[clashing_cluster_list], new_y_coords[clashing_cluster_list], np.zeros(len(clashing_cluster_list),dtype=np.float32))))
...
#            print 'finding correct indices took:  {} seconds'.format(indextime - checkdiststime)
        ...
        if very_verbose and iteration >2:
            print('Clashing clusters after translation and rotation - more than two iterations: ',clashing_clusters,'\r', end=' ')
        iteration += 1
        if iteration >10000:
            print('Clashing clusters after translation and rotation - >1000 iterations: ',clashing_clusters,'\r', end=' ')
...
#        print zip(all_dists_btwn_prots, all_sizes)
    ...
    if report_timings:
        currentTime6 = time.time()
        print('sorting out clashes ({} iterations)'.format(iteration)+' took:  {} seconds\r'.format(currentTime6 - currentTime5), end=' ')
        ...
        if t % output_frequency == 0:
            ...
            if  'trr' in write:
                ...
                for ts in u.trajectory[timestep:timestep+1]:
                    AllProts.set_positions(list(zip(x_coords, y_coords, [0.5 for i in range(n_particles)])))
                    AllProts.set_velocities(list(zip(angles, [0.0 for i in range(n_particles)], [0.0 for i in range(n_particles)])))
            ...
            print('{} steps took: {} seconds\r'.format(output_frequency, newCurrentTime-currentTime), end=' ')
